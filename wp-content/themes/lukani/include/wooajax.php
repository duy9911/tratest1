...
add_action('wp_head','lukani_woo_ajaxurl');
function lukani_woo_ajaxurl() {
?>
...
function product_quickview() {
	...
	<div class="woocommerce product">
		<div class="product-images">
			...
			<div class="main-image images"><img src="<?php echo esc_attr($image_link);?>" alt="<?php echo esc_attr_e('image','lukani') ?>" /></div>
			<?php
		...
		<div class="product-info">
			...
			<a class="see-all" href="<?php echo esc_url($product->get_permalink()); ?>"><?php echo esc_html($lukani_opt['quickview_link_text']); ?></a>
			<div class="quick-add-to-cart">
				<?php woocommerce_template_single_add_to_cart(); ?>
			</div>
			<div class="quick-desc"><?php echo do_shortcode(get_post($productid)->post_excerpt); ?></div>
			<?php if( function_exists('lukani_product_sharing') ) { ?>
				<div class="social-sharing"><?php lukani_product_sharing(); ?></div>
			<?php } ?>
		</div>
	</div>
	<?php
...
function get_cartinfo() {
	...
	echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'lukani'), $woocommerce->cart->cart_contents_count);
...
function get_productinfo() {
	...
	$product = wc_get_product( $productid );
	?>
	<h3><?php esc_html_e('Product is added to cart', 'lukani');?></h3>
	...
	<div class="buttons">
		<a class="button" href="<?php echo get_permalink( wc_get_page_id( 'cart' ) );?>"><?php esc_html_e('View Cart', 'lukani');?></a>
	</div>
	<?php
	die();
}
